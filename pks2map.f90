...
11   format('Usage: pks2map <filein> <fileout> <tablefile> <profilecode> [<zmin> <nside> <scramble> <center> <npix> <fov> <zmax> <chihview> <model> <PSZcut>]  profile choices: tsz, ksz, kap, tau, tco, thi')
  ...
  if(profilecode == 'tsz') profile = itsz
  if(profilecode == 'ksz') profile = iksz
  if(profilecode == 'kap') profile = ikap
  if(profilecode == 'tau') profile = itau
  if(profilecode == 'tco') profile = itco
  if(profilecode == 'thi') profile = ithi
  ...
  if ((profile ==  itco) .or. (profile ==  ithi)) then
     num_nuobs = int((nuobs_i - nuobs_f)/dnu)
  else
     num_nuobs = 1
  endif
...
14 format('cut_low, cut_high:  ',f4.2,1x,f4.2,' Gpc')
  ...
  if(scramble==1) call scramble_halos
  if(centre==1  ) call center_halos
  ...
  call mpi_reduce(nhalol,nhalomap,1,mpi_int,mpi_sum,0,mpi_comm_world,ierr)
  call mpi_reduce(meantaul,meantau,1,mpi_double_precision,mpi_sum,0,mpi_comm_world,ierr)

  ! Gather maps from processors
  call mpi_reduce(fsmapl,fsmap,npix**2,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
  call mpi_reduce(hpmapl,hpmap,hpnpix, mpi_real,mpi_sum,0,mpi_comm_world,ierr)
